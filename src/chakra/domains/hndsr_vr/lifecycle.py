...
class LifecycleHooks:
    ...
    def render_notebook(self, version: str, parent: str | None, paths: VersionPaths) -> str:
        ...
        run_control = f'''subprocess.run([sys.executable, "-m", "chakra.hndsr_vr.evaluate_runner", "--config", CONTROL_CONFIG, "--run-name", f"{{VERSION}}-control"], cwd=REPO_ROOT, env=ENV, check=True)'''
        run_smoke = f'''subprocess.run([sys.executable, "-m", "chakra.hndsr_vr.train_runner", "--config", SMOKE_CONFIG, "--run-name", f"{{VERSION}}-smoke"], cwd=REPO_ROOT, env=ENV, check=True)\nsubprocess.run([sys.executable, "-m", "chakra.hndsr_vr.evaluate_runner", "--config", SMOKE_CONFIG, "--run-name", f"{{VERSION}}-smoke-eval", "--checkpoint", f"artifacts/{{VERSION}}-smoke/checkpoints/{version}_smoke_best.pt"], cwd=REPO_ROOT, env=ENV, check=True)'''
        run_train = f'''subprocess.run([sys.executable, "-m", "chakra", "validate-version", "--version", VERSION], cwd=REPO_ROOT, env=ENV, check=True)\nsubprocess.run([sys.executable, "-m", "chakra.hndsr_vr.train_runner", "--config", TRAIN_CONFIG, "--run-name", f"{{VERSION}}-train"], cwd=REPO_ROOT, env=ENV, check=True)\nsubprocess.run([sys.executable, "-m", "chakra.hndsr_vr.evaluate_runner", "--config", TRAIN_CONFIG, "--run-name", f"{{VERSION}}-eval", "--checkpoint", f"artifacts/{{VERSION}}-train/checkpoints/{version}_train_best.pt"], cwd=REPO_ROOT, env=ENV, check=True)'''
        ...
        notebook = {
            "cells": [
                ...
                {"cell_type": "markdown", "metadata": {}, "source": [f"## Experiment Registry\n\n- Version: `{version}`\n- Parent: `{parent or 'none'}`\n- Lineage: `scratch`\n- Commands: `python -m chakra validate-version --version {version}`, `python -m chakra.hndsr_vr.train_runner --config`, `python -m chakra.hndsr_vr.evaluate_runner --config`\n"]},
                ...
                {"cell_type": "code", "execution_count": None, "metadata": {}, "outputs": [], "source": [f"subprocess.run([sys.executable, '-m', 'chakra', 'validate-version', '--version', '{version}'], cwd=REPO_ROOT, env=ENV, check=True)\n"]},
