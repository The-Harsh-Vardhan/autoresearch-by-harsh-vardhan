...
def test_gpt_nano_forward():
    from chakra.domains.nlp_lm.models import GPTNano
...
def test_bigram_forward():
    from chakra.domains.nlp_lm.models import BigramBaseline
...
def test_char_dataset():
    from chakra.domains.nlp_lm.dataset import CharTextDataset
...
def test_metrics_shapes():
    from chakra.domains.nlp_lm.metrics import calculate_bpb, calculate_cross_entropy, calculate_perplexity
...
def test_nlp_version_helpers():
    ...
    from chakra.domains.nlp_lm.utils import version_stem, version_slug
    assert version_stem("v1.0") == "v1.0_NLP_LM"
    assert version_slug("v1.0") == "v10-nlp-lm"


def test_nlp_lifecycle_hooks_resolve_paths():
    from chakra.domains.nlp_lm.lifecycle import LifecycleHooks
